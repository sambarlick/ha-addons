        ...
        with GPSDClient(host="127.0.0.1") as gps_client:
            ...
            for raw_result in gps_client.json_stream():
                ...
                if result.get("class") == "SKY":
                    ...
                    if n_sat == 0 and current_state["fix_status"] == "Connected": continue
                    sat_payload = {
                    ...
                    if mode < 2:
                        ...
                        # CRITICAL FIX: IF we are stuck at 0.0 (Startup), FORCE an update
                        startup_override = (current_state["latitude"] == 0.0 and current_state["longitude"] == 0.0)

                        if is_moving or startup_override:
                            ...
                            # PARKED (And already initialized) -> Lock Lat/Lon
                        ...
                        if "time" in result:
                            try:
                                ts = result["time"]
                                if "T" in ts: current_state["time_str"] = ts.split("T")[1].split(".")[0] 
                            except: pass
